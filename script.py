...
def all_branches(url):
    ...
    cookies = {
        ...
        'cookie': '__cfduid=d63aeeb70667f7b04f2403b1fb5815d3f1609152427; _ga=GA1.2.1773745497.1609152430; _gid=GA1.2.1847551795.1611254247; ci_session=013b0624359c465613beed87d547ed3dd70af9d0'

    }
    retries = 0
    while True:
        try:
            r = requests.post('https://dcciinfo.ae/sitefront/clientcompanies/getBranchesBlockHtml',
            ...
            retries += 1
            if retries < 200:
                print("Can't connect the server...retrying in three seconds...")
                time.sleep(3)
                continue
            else:
                print(
                    "\nCan't connect the server...please make sure there's a reliable connection and try again\nClosing...")
                sys.exit()
    ...
    for detail in secondary_details:
        try:
            data['Phone'] = detail.find(
                'div', class_='col-9 col-md-4').a['href'].split(':')[-1]
        except:
            data['Phone'] = ''
        try:
            data['Box'] = detail.find(
                'div', itemprop='postOfceBoxNumber').text.strip()
        except:
            data['Box'] = ''
        try:
            data['Area'] = detail.find(
                'div', class_='col-9 col-md-10').text.strip()
        except:
            data['Area'] = ''
        try:
            data['Address'] = detail.find(
                'div', itemprop='addressRegion').text.strip()
        except:
            data['Address'] = ''
        try:
            data['Fax'] = detail.find_all(
                'div', class_='col-9 col-md-4')[1].text.strip()
        except:
            data['Fax'] = ''
        try:
            data['City'] = detail.find(
                'div', itemprop='addressLocality').text.strip()
        except:
            data['City'] = ''
        ...
        for company in companies:
            ...
            while True:
                try:
                    ...
                    if retries < 200:
                        print(
                            "Can't connect the server...retrying in three seconds...")
                        time.sleep(3)
                        continue
                    else:
                        print(
            ...
            try:
                ...
                pass
            try:
                data['Last Updated'] = secondary_details_1.find(
                    'div', class_='col-12').text.split(' - ')[-1]
            except:
                data['Last Updated'] = ''
            try:
                data['Phone'] = secondary_details_1.find(
                    'div', class_='col-9 col-md-4').a['href'].split(':')[-1]
            except:
                data['Phone'] = ''
            try:
                data['Box'] = secondary_details_1.find(
                    'div', itemprop='postOfceBoxNumber').text.strip()
            except:
                data['Box'] = ''
            try:
                data['Area'] = secondary_details_1.find(
                    'div', class_='col-9 col-md-10').text.strip()
            except:
                data['Area'] = ''
            try:
                data['Website'] = secondary_details_1.find(
                    'a', class_='websiteClick').text.strip()
            except:
                data['Website'] = ''
            try:
                data['Address'] = secondary_details_1.find(
                    'div', itemprop='addressRegion').text.strip()
            except:
                data['Address'] = ''
            try:
                data['Fax'] = secondary_details_1.find_all(
                    'div', class_='col-9 col-md-4')[1].text.strip()
            except:
                data['Fax'] = ''
            try:
                data['City'] = secondary_details_1.find(
                    'div', itemprop='addressLocality').text.strip()
            except:
                data['City'] = ''
            try:
                data['Physical Location'] = secondary_details_2.find(
                    'a', class_='locationClicks')['href'].split('/')[-1]
            except:
